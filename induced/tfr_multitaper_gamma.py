...
n_runs = 5  
sfreq = 1200  
freqs = np.arange(30, 100, 1.0)  # Frequency range from 30 to 100 Hz  
n_cycles = freqs / 2  # Different number of cycles per frequency  
time_bandwidth = 6  # Time bandwidth  
...
for ss in range(1, n_subjects + 1):  
    ...
    for rr in range(1, n_runs + 1):  
        try:
            # Load data  
            file_path = f"/d/mjt/9/projects/OPM-Analysis/OPM_retest/data/derivatives/retest_gamma_analysis_revised_2025-02-07_v7.0.0/sub-RP0{ss}/ses-00{rr}/opm/sub-RP0{ss}_ses-00{rr}_ts_pca_5mm.mat"  
...
mean_envelope = [np.mean(inst,1) for inst in tfr_data_arrays]
...
for ss in range(5):
    ...
    for rr in range(5):
        ...
        max_amp = np.max(mean_power[ss][rr,freq_idx])
        max_freq = freqs[np.argmax(mean_power[ss][rr,freq_idx])]
...
#%% CORRELATION
corrmat = np.zeros(shape=(25,25))
mean_power_cat = np.vstack(mean_power)

for ss in range(25):
    for ss2 in range(25):
        r = pearsonr(mean_power_cat[ss,:],mean_power_cat[ss2,:])
        corrmat[ss,ss2] = r.statistic
